...
# Trackers
score:                      .byte 0
last_move:                  .byte -1  # This will become a char ASCII
...
_start:
    ...
    mv s10, a0  # Provide to $s10 the number of players

    # FIRST, STORE ALL INITIAL COORDS INTO MEMORY FOR ALL PLAYERS
    # Assert num of players > 0
    # Prepare args.
    # Recall that $a0 contains num of players
    # Load Character
    la a1, character
    la a2, initial_character
    # Load Box
    la a3, box
    la a4, initial_box
    # Load Target
    la a5, target
    la a6, initial_target

    jal initialize_player_object_coords
    # Returns $a0, the address of where player object coords
    # begin to be stored
    mv s0, a0  # $s0 tracks that starting address

    # Assert that the object coords of each player
    # is now stored in the stack memory.

    # SECOND, PLAY EACH PLAYER'S GAME AND KEEP TRACK OF THEIR SCORES
    # New incrementor
    li t0, 0
    # Assert $s10 is the number of players
    RUN_ALL_GAMES:
    bge t0, s10, LEADERBOARD

    # Fully run the game for each player, before moving onto the next (if any).
    # Track the scores of the players and store them into the stack (to build the leaderboard later)
    ...
    # From the stack, load the current player's object coords into
    # those of the data memory.
    
    # Assert $s0 is the correct place in memory
    mv sp, s0  # to point to the place in memory where player object coords begin
    
    # CHARACTER
    la a2, character
    jal replace_data_memory_coordinate
    ...
    # INITIAL_TARGET
    la a2, initial_target
    jal replace_data_memory_coordinate

    # Assert all object coords are updated to this player's version of them

    # Now, run the game:
    jal GAME_LOOP

    # Assert that the score of this player is stored in data memory

    # Then, shove it into the stack memory
    mv sp, 

    # Load it into the stack memory
...
# Stores the initial game data for every player's object coordinates
# into the stack, needed for multiplayer mode.
#
# This allows every player to be playing the same game as initially generated.
#
# Additionally, the score of this player
#
# Arguments:
# - $a0, the number of players 
# - $a1, the character address
# - $a2, the initial character address
# - $a3, the box address
# - $a4, the initial box address
# - $a5, the target address
# - $a6, the initial target address
# Returns:
# - $a0, the address of where player object coords begin to be stored
initialize_player_object_coords:
    mv t2, a0  # note the number of games to make
    li t5, -2  # $t5 is how much to move between chunks of memory in the stack
    li t0, 0  # loop incrementor (include the first player)
    
    # Stash $ra
    addi sp, sp, -4
    sw ra, 0(sp)

    # Note of the stack pointer address
    mv t6, sp

    addi sp, sp, -16  # avoid colliding with function calls
    # -16 = 4(-4) = skip past (4 - 1 = 3) nested function calls

    MAKE_GAMES:
    bge t0, t2, DONE_MAKING_GAMES

    # Assert that $sp initially points to a usable space in memory.
    # Now, store all coordinates:
    
    # SCORE:
    sb zero, 0(sp)  # initialize a score of 0 for this player
    addi sp, sp, -1  # move on

    # CHARACTER:
    mv a0, a1
    jal shove_coords_into_memory
    add sp, sp, t5  # move to the next chunk of memory

    # INITIAL_CHARACTER:
    mv a0, a2
    jal shove_coords_into_memory
    add sp, sp, t5  # move through memory
    ...
    # AFTER THE LOOP:
    # Restore the stack pointer to the noted address from earlier
    mv sp, t6
    ...
    # Return
    mv a0, t6  # address of where player object coords begin to be stored
    jr ra


# Takes an object's coords and shoves it into
# the current stack pointer's location in memory.
# Arguments: $a0, the object coordinate memory address
shove_coords_into_memory:
    # Grab from arg. and shove
    lb t3, 0(a0)  # grab row from object address
    sb t3, 0(sp)  # store row into stack
    lb t3, 1(a0)  # grab column from object address
    sb t3, 1(sp)  # store column into stack

    # Return
    jr ra


# Updates the object coordinates in the data memory
# to reflect that of some player.
# Arguments: $a2, the object memory address
# Preconditions:
# - $sp is set to the correct location to grab a coordinate.
replace_data_memory_coordinate:
    li t1, -2  # the distance to move between the object coords in memory

    # RETRIEVE OBJECT COORDS FROM STACK MEMORY INTO REGISTERS;
    # THEN, PUT REGISTER VALUES INTO DATA MEMORY!

    # CHARACTER
    lb a0, 0(sp)
    lb a1, 1(sp)
    # Assert $a0 and $a1 contain the row and column
    # Assert $a2 contains the object memory address
    sb a0, 0(a2)  # store obtained row
    sb a1, 1(a2)  # store obtained column
    addi sp, sp, t1  # move to the next coordinate in memory
